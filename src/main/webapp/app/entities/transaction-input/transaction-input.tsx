...
export const TransactionInput = (props: RouteComponentProps<{ url: string }>) => {
  ...
  const totalItems = useAppSelector(state => state.transactionInput.totalItems);
  ...
  return (
    <div>
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="chainApp.transactionInput.id">ID</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('transactionOutputId')}>
                  <Translate contentKey="chainApp.transactionInput.transactionOutputId">Transaction Output Id</Translate>{' '}
                  <FontAwesomeIcon icon="sort" />
                </th>
                <th className="hand" onClick={sort('uTXO')}>
                  <Translate contentKey="chainApp.transactionInput.uTXO">U TXO</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th>
                  <Translate contentKey="chainApp.transactionInput.transaction">Transaction</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                <th />
              </tr>
            </thead>
                ...
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {transactionInput.transaction ? (
                      <Link to={`transaction/${transactionInput.transaction.id}`}>{transactionInput.transaction.id}</Link>
                    ) : (
                      ''
                    )}
                  </td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${transactionInput.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${transactionInput.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
      ...
      {totalItems ? (
        <div className={transactionInputList && transactionInputList.length > 0 ? '' : 'd-none'}>
